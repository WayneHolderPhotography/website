...
class OCDI_Helpers {
	...
	public static function download_import_files( $import_file_info, $start_date = '' ) {
		...
		// Get Theme Options Files. If defined!
		if ( ! empty( $import_file_info['import_theme_options_url'] ) ) {

			// Retrieve widget content from the URL.
			$demo_import_options_content = self::get_content_from_url( $import_file_info['import_theme_options_url'], $import_file_info['import_file_name'] );

			// Return from this function if there was an error.
			if ( is_wp_error( $demo_import_options_content ) ) {
				return $demo_import_options_content;
			}

			// Setup filename path to save the widget content.
			$import_options_file_path = $upload_path . apply_filters( 'pt-ocdi/downloaded_import_file_prefix', 'demo-import-file_' ) . $start_date . apply_filters( 'pt-ocdi/downloaded_widgets_file_suffix_and_file_extension', '.txt' );

			// Write widget content to the file and return the file path on successful write.
			$downloaded_files['options'] = $demo_import_options_content;
			
			// Return from this function if there was an error.
			if ( is_wp_error( $downloaded_files['options'] ) ) {
				return $downloaded_files['options'];
			}
		}
	...
	public static function import_file_info( $selected_import_files ) {
		...
		sprintf(
			__( 'Files info:%1$sSite URL = %2$s%1$sData file = %3$s%1$sWidget file = %4$s%1$sTheme Options file = %5$s', 'pt-ocdi' ),
			PHP_EOL,
			get_site_url(),
			...
			empty( $selected_import_files['options'] ) ? esc_html__( 'not defined!', 'pt-ocdi' ) : $selected_import_files['options']
		);
	}
